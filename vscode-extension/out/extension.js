...
function activate(context) {
    const selector = [{ language: 'shiden', scheme: 'file' }, { pattern: '**/*.sd' }];
    const provider = {
    ...
    context.subscriptions.push(vscode.workspace.onWillSaveTextDocument(async (e) => {
        ...
        if (!doc.fileName.endsWith('.sd') && doc.languageId !== 'shiden') {
            return;
        }
        const edits = await provider.provideDocumentFormattingEdits(doc, { insertSpaces: true, tabSize: 4 }, new vscode.CancellationTokenSource().token);
        if (!edits || edits.length === 0) {
    ...
    function findUnquotedForCheck(s, pat) {
        let inQuote = false;
        let quoteChar = '';
    ...
    function computeDiagnosticsFor(doc) {
        const diags = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (isLet) {
                let ok = false;
                for (const t of allowedTypeSuffixes) {
                    if (line.trimEnd().endsWith('/' + t)) {
                        ok = true;
                        break;
                    }
                }
    ...
    function refreshDiagnosticsFor(doc) {
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((doc) => {
        ...
        if (useCli) {
            runShidenCheckFor(doc);
        }
        else {
            refreshDiagnosticsFor(doc);
        }
    }));
    async function runShidenCheckFor(doc) {
        const enabled = vscode.workspace.getConfiguration('shiden').get('diagnostics.enabled', true);
        if (!enabled) {
            diagCollection.delete(doc.uri);
            return;
        ...
        try {
            ...
            cp.exec(cmd, { cwd: workspaceFolder, timeout: 5000 }, (err, stdout, stderr) => {
                ...
                try {
                    ...
                    if (idx !== -1) {
                        try {
                            parsed = JSON.parse(output.slice(idx));
                        }
                        catch (e2) {
                            parsed = null;
                        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('shiden.runCheck', async (uri) => {
        let doc;
        if (uri) {
        ...
        if (doc && (doc.fileName.endsWith('.sd') || doc.languageId === 'shiden')) {
            runShidenCheckFor(doc);
        }
    }));
    ...
    function collectFunctionSignatures(doc) {
        const out = new Map();
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument((d) => { if (d.fileName.endsWith('.sd')) {
        refreshWorkspaceFunctionIndex();
    } }));
    context.subscriptions.push(vscode.workspace.onDidChangeWorkspaceFolders(() => { refreshWorkspaceFunctionIndex(); }));
    function collectLocalSymbols(document, position) {
    ...
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider(selector, {
        async provideCompletionItems(document, position) {
            ...
            for (const wf of workspaceFunctions) {
                if (wf.file === document.uri.fsPath) {
                    continue;
                }
        ...
        resolveCompletionItem(item) { return item; }
    }, '/', '(', ','));
}
...
function formatSource(src) {
    ...
    for (let raw of lines) {
        ...
        if (isLetLine) {
            ...
            if (!ok) {
                if (line.trimEnd().endsWith('/')) {
                    line = line.trimEnd().replace(/\/$/, '/unit');
                }
                else {
                    line = line.trimEnd() + '/unit';
                }
            }
        }
        else {
